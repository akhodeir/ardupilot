...
// Static semaphore for ArduPilot-based SPI bus locking (replaces ESP-IDF race-prone system)
AP_HAL::Semaphore *SPIDevice::_spi_bus_semaphore = nullptr;
    ...
    , _bus_acquired(false)  // Initialize ArduPilot semaphore state
{
    ...
    // Initialize shared SPI bus semaphore once
    if (_spi_bus_semaphore == nullptr) {
        _spi_bus_semaphore = new ESP32::Semaphore();
    }
...
void SPIDevice::acquire_bus(bool acquire)
{
    ...
    if (acquire && !_bus_acquired) {
        ...
        if (_spi_bus_semaphore) {
            _spi_bus_semaphore->take_blocking();
            _bus_acquired = true;
        }

        // Still use ESP-IDF for the actual SPI transaction, but bypass their locking
        // Use spi_device_transmit() directly without acquire/release calls

    } else if (!acquire && _bus_acquired) {
        // Release ArduPilot semaphore
        if (_spi_bus_semaphore) {
            _spi_bus_semaphore->give();
            _bus_acquired = false;
        }
    }
}
